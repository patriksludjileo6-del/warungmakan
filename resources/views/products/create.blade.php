...
<head>
    ...
    <title>Tambah Menu Baru - Warung Nusantara</title>
    <style>
...
<body>
    <div class="card">
        <div class="header">
            <div>
                <div class="subtitle">Input Data Baru</div>
                <div class="title">Tambah Menu Kuliner</div>
            </div>
        ...
        <form action="{{ route('products.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Nasi Goreng Spesial" class="form-input" required>
            </div>

            <div class="form-group">
                <label class="form-label">Deskripsi & Komposisi</label>
                <textarea name="description" rows="3" placeholder="Jelaskan rasa atau bahan utama..." class="form-input" style="resize: vertical;">{{ old('description') }}</textarea>
            </div>

            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Harga Jual (Rp)</label>
                    <input type="number" step="0.01" name="price" value="{{ old('price') }}" placeholder="15000" class="form-input" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Stok Awal (Porsi)</label>
                    <input type="number" name="stock" value="{{ old('stock', 25) }}" placeholder="25" class="form-input" required>
                </div>
            </div>

            <div class="footer-actions">
                <a href="{{ route('products.index') }}" class="btn-cancel">Batal</a>
                <button type="submit" class="btn-submit">Simpan Menu Baru</button>
            </div>
        </form>
    </div>
</body>
